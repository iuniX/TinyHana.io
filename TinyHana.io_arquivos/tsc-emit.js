...
(function (game) {
    ...
    var CamMovement = /** @class */ (function (_super) {
        ...
        CamMovement.prototype.OnUpdate = function () {
            ...
            this.world.forEach([game.Limits, game.TeleportLimit, game.Speed, ut.Core2D.TransformLocalPosition], function (limits, teleportLimit, speed, position) {
                ...
                if (ut.Runtime.Input.getKey(ut.Core2D.KeyCode.D))
                    localPosition.x += speed.speed * dt;
                localPosition.y = Math.max(limits.minY, Math.min(limits.maxY, localPosition.y));
                if (localPosition.x >= teleportLimit.maxX) {
                    localPosition.x = teleportLimit.minX;
                }
                else if (localPosition.x <= teleportLimit.minX) {
                    localPosition.x = teleportLimit.maxX;
                }
                position.position = localPosition;
            });
        };
        CamMovement = __decorate([
